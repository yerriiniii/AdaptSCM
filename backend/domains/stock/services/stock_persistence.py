...
from domains.stock.services.stock_aggregate import (
    _detect_country,
...
def _resolve_country_code(
    ...
    if file_countries and idx < len(file_countries):
        ...
        if file_countries[idx] and override_country is None:
            raise HTTPException(
                status_code=400,
                detail=f"{upload_file.filename}: ??? ??????? 2~4?????? ?????",
            )
...
def _parse_optional_date(raw_value: str | None, filename: str) -> date | None:
    ...
    if pd.isna(parsed):
        raise HTTPException(status_code=400, detail=f"{filename}: ??? ??????????? ??????.")
    return parsed.date()
...
def _delete_s3_object(bucket: str | None, key: str | None) -> None:
    ...
    if not is_s3_configured():
        logger.warning("S3 ????????? ?????????????(bucket=%r, key=%r).", bucket, key)
        return
    if not bucket or not key:
        logger.warning("S3 bucket ??? key? ??? ?????????????(bucket=%r, key=%r).", bucket, key)
        return
    ...
    try:
        ...
        logger.warning(
            "S3 delete_object ???(??): bucket=%r key=%r err=%s",
            bucket,
            key,
            exc,
            exc_info=True,
...
def _delete_s3_prefix(bucket: str | None, prefix: str) -> int:
    ...
    try:
        ...
        logger.warning(
            "S3 prefix ???? ???(??): bucket=%r prefix=%r err=%s",
            bucket,
            prefix,
            exc,
            exc_info=True,
...
def _persist_dataframe(
    ...
    *,
    ...
    apply_db_locale_columns_to_raw_stock_df(db, df, country_code)
...
def list_stock_files(db: Session) -> list[dict]:
...
def patch_stock_file_base_date(db: Session, file_id: str, base_date_value: str | None) -> dict:
    """????????????? ??????????DB???(InventoryAggregate)???? ????"""
    try:
        uid = uuid.UUID(str(file_id).strip())
    except ValueError as exc:
        raise HTTPException(status_code=400, detail="file_id? ?????? ??????.") from exc
...
def get_stock_view(db: Session, country_code: str) -> dict:
    ...
    # ???? ????? ????? SKU???? ??? ?????????????????????? ??? ??? ??? (????? ??? ??????????)
    apply_db_locale_columns_to_stock_merged(db, frame)
    frame = (
        frame.groupby(
    ...
    attach_item_mapping_kr_to_stock_rows(db, normalized_country, result_rows)
    return {
...
def persist_stock_uploads(
    ...
    if not is_s3_configured():
        raise HTTPException(status_code=500, detail="S3 ???????? ??? ?? ??????? ????????")
    if not settings.s3_bucket:
        raise HTTPException(status_code=500, detail="S3_BUCKET????????? ????????")
    ...
    try:
        for idx, upload_file in enumerate(files):
            ...
            if not raw_bytes:
                raise HTTPException(status_code=400, detail=f"{upload_file.filename}: ??????? ??????.")

            df = _read_stock_file(upload_file).copy()
...
def prepare_stock_direct_uploads(files: list[dict], db: Session | None = None) -> list[dict]:
    settings = get_runtime_settings()
...
def complete_stock_direct_uploads(db: Session, files: list[dict]) -> list[dict]:
    settings = get_runtime_settings()
    if not settings.s3_bucket or not is_s3_configured():
        raise HTTPException(status_code=500, detail="S3 ???????? ??? ?? ??????? ????????")
    ...
    try:
        for file in files:
            ...
            if not original_name:
                raise HTTPException(status_code=400, detail="original_name??????????")
            country_code = _normalize_country_code(file.get("country_code")) or "KR"
            ...
            if not stored_name or not s3_key:
                raise HTTPException(status_code=400, detail=f"{original_name}: ???????? ???? ??????.")
            ...
            df = _read_stock_bytes(original_name, raw_bytes).copy()
            row_frame = _build_row_frame(df)
            validate_stock_row_frame_item_mapping(
                db,
                row_frame,
...
def delete_stock_file(db: Session, file_id: str) -> None:
    try:
...
def delete_stock_files(db: Session, country_code: str | None = None) -> dict[str, int]:
    settings = get_runtime_settings()
