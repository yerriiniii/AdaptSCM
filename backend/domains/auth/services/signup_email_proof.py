...
from domains.auth.services.auth_email import send_signup_email_proof_link
...
def register_user_with_proof(
    ...
    *,
    ...
    u = db.execute(select(UserAccount).where(UserAccount.email == email)).scalar_one_or_none()
    if u is not None:
